import{y as dt,g as Ke,z as pt,M as Ne,a as g,t as mt,i as ht,D as ft,n as me,v as U,B as De,C as Oe,E as Me,x as gt,p as yt,F as vt,j as bt,G as Fe,f as be,P as we,A as C,m as l,l as he,k as wt,H as je}from"./shared-BYbRkO1_.js";const Ie=`__js_AQP_ACCEPTED
...
>
  ...
  ></iframe>
</div>`,G=20;let Ue=0;function ge(e={}){return Ue+=1,{key:`custom-item-${Ue}`,name:"",prompt:"",lowAnchor:"",highAnchor:"",simpleExplanation:"",reverseScored:!1,...e}}function ze(){return{language:"en-GB",name:"",shortName:"",version:"1.0.0",description:"A researcher-supplied questionnaire.",introPrompt:"Answer each item about the task that you have just completed.",sourceLabel:"Researcher-supplied questionnaire",sourceUrl:"",scaleType:"agreement",minimum:1,maximum:5,step:1,scoreName:"Questionnaire score",aggregation:"mean",items:[ge({name:"Item 1"}),ge({name:"Item 2"})]}}function x(e,t){const n=e.trim().replace(/\s+/g," ");if(!n)throw new Error(`${t} is required.`);return n}function It(e){return e.trim().toLowerCase().normalize("NFKD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"").slice(0,50)}function At(e,t){const n=t.trim()||e.trim();return n.length<=240?n:`${n.slice(0,237).trimEnd()}...`}function Ge(e){if(!Number.isInteger(e.minimum)||!Number.isInteger(e.maximum)||!Number.isInteger(e.step))throw new Error("Scale minimum, maximum and step must be whole numbers.");if(!Array.isArray(e.items)||e.items.length<1||e.items.length>G)throw new Error(`Add between 1 and ${G} questionnaire items.`);const t=x(e.name,"Questionnaire name"),n=x(e.shortName,"Short name"),i=It(n||t);if(!i)throw new Error("Questionnaire name must contain at least one Latin letter or number for its stable ID.");const r=e.items.map((p,d)=>{const S=x(p.prompt,`Item ${d+1} question`),c=p.simpleExplanation.trim().replace(/\s+/g," ");return{id:`item-${String(d+1).padStart(2,"0")}`,name:x(p.name,`Item ${d+1} label`),prompt:S,...c?{simpleExplanation:c}:{},shortMeaning:At(S,c),lowAnchor:x(p.lowAnchor,`Item ${d+1} low endpoint`),highAnchor:x(p.highAnchor,`Item ${d+1} high endpoint`),...p.responseLabels?{responseLabels:p.responseLabels}:{}}}),s=e.items.flatMap((p,d)=>p.reverseScored?[`item-${String(d+1).padStart(2,"0")}`]:[]),a=r.every(p=>p.simpleExplanation),v=e.aggregation==="mean"?e.minimum:e.minimum*r.length,f=e.aggregation==="mean"?e.maximum:e.maximum*r.length,u={schemaVersion:1,language:x(e.language,"Questionnaire language"),id:`custom-${i}`,version:x(e.version,"Questionnaire version"),name:t,shortName:n,description:x(e.description,"Questionnaire description"),introPrompt:x(e.introPrompt,"Participant instruction"),officialContentNotice:"This questionnaire definition was supplied by the study conductor. Its wording, use and interpretation must match the approved study protocol.",source:{label:x(e.sourceLabel,"Source or authorship label"),...e.sourceUrl.trim()?{url:e.sourceUrl.trim()}:{}},scale:{type:e.scaleType,minimum:e.minimum,maximum:e.maximum,step:e.step},items:r,scoring:{strategy:e.aggregation==="mean"?"mean-v1":"sum-v1",scoreName:x(e.scoreName,"Score name"),minimum:v,maximum:f,...s.length?{reverseItemIds:s}:{}},supports:{simplerExplanations:a,smileyLandmarks:!1}};return Ze(u)}function Ze(e){const t=dt(e);if(Ke(t.id))throw new Error("A custom questionnaire cannot replace a built-in questionnaire ID.");if(!t.id.startsWith("custom-"))throw new Error("A custom questionnaire ID must start with custom-.");if(t.scoring.strategy!=="mean-v1"&&t.scoring.strategy!=="sum-v1")throw new Error("A custom questionnaire must use the reviewed mean or sum scorer.");if(t.pairwise)throw new Error("Custom questionnaires do not support pairwise comparisons.");if(t.landmarks||t.supports.smileyLandmarks)throw new Error("Custom questionnaires do not support smiley landmarks.");const n=pt(t);if(n>Ne)throw new Error(`The questionnaire definition is ${n} bytes; the participant-link limit is ${Ne} bytes.`);return t}function Ct(e){return`${e.id}-${e.version.replace(/[^A-Za-z0-9._-]+/g,"-")}.questionnaire.json`}const $t=2e6,Ce=/<\s*(?:script|iframe|object|embed|style)\b|(?:href|src)\s*=\s*["']?\s*javascript:|\son[a-z]+\s*=/i,$e=/<\s*(?:img|picture|video|audio|canvas|svg|math|form|input|button|select|textarea|table)\b/i,qe=/\$\{(?:e|q|lm|gr)?:?\/?\/|q:\/\/|\{(?:if|TOKEN|INSERTANS|[A-Za-z][A-Za-z0-9_.]*\.(?:NAOK|shown))\b/i;function E(e){return e&&typeof e=="object"&&!Array.isArray(e)?e:null}function $(e){return typeof e=="string"||typeof e=="number"?String(e):""}function A(e){return e.replace(/\s+/g," ").trim()}function I(e,t){e.confirmationCodes.has(t.code)||(e.confirmationCodes.add(t.code),e.confirmations.push(t))}function B(e,t,n,i){const r=$(e);if(!r.trim())return i.unsupported.push({code:"missing-visible-text",title:`${t} is empty`,detail:`${n} does not contain participant-visible text.`}),null;if(Ce.test(r)||qe.test(r))return i.unsupported.push({code:"unsafe-dynamic-content",title:`${t} contains executable or dynamic content`,detail:`${n} contains script, an event handler, a JavaScript URL or survey-expression text. The importer does not execute or approximate it.`}),null;if($e.test(r))return i.unsupported.push({code:"unsupported-structured-content",title:`${t} contains media, a table or an interactive control`,detail:`${n} cannot be represented as one safe plain-text rating item without changing participant-visible content.`}),null;const a=new DOMParser().parseFromString(`<body>${r}</body>`,"text/html"),v=A(a.body.textContent??"");return v?((/<[^>]+>/.test(r)||v!==A(r))&&I(i,{code:"plain-text-normalisation",title:"Formatting was converted to plain text",detail:"Imported wording is rendered as safe plain text. Review it against the source export before use."}),v):(i.unsupported.push({code:"missing-visible-text",title:`${t} has no readable text`,detail:`${n} contains no participant-visible text after safe text extraction.`}),null)}function ne(e){const t=typeof e=="number"?e:Number($(e).trim());return Number.isInteger(t)?t:null}function se(e){if(e.length<2||e.some(n=>n<0||n>100)||new Set(e).size!==e.length)return!1;const t=e[1]-e[0];return t>0&&e.every((n,i)=>i===0||n-e[i-1]===t)}function Xe(e,t){if(!Array.isArray(e)||e.length!==Object.keys(t).length)return null;const n=e.map(String);return new Set(n).size===n.length&&n.every(i=>i in t)?n:null}function et(e,t,n,i){const r=E(e);return B(r?.Display,t,n,i)}function qt(e){return e.flatMap(n=>n.labels).map(n=>n.toLowerCase()).some(n=>n.includes("agree")||n.includes("disagree"))?"agreement":"semantic-differential"}function Tt(e){const t=e.split(/\s+/).map(i=>i.replace(/[^A-Za-z0-9]/g,"")[0]??"").join("").slice(0,12);return t?t.toUpperCase():e.replace(/[^A-Za-z0-9]+/g,"").slice(0,12)||"IMPORTED"}const Et={SQI:"sq",AR:"ar",HYE:"hy",ASM:"as","AZ-AZ":"az-AZ",ID:"id",MS:"ms",BEL:"be",BN:"bn",BS:"bs","PT-BR":"pt-BR",BG:"bg",CA:"ca",CEB:"ceb","ZH-S":"zh-CN","ZH-T":"zh-TW",HR:"hr",CS:"cs",DA:"da",NL:"nl","EN-GB":"en-GB",EN:"en",EO:"eo",ET:"et",FI:"fi",FR:"fr","FR-CA":"fr-CA",KAT:"ka",DE:"de",EL:"el",GU:"gu",HE:"he",HI:"hi",HIL:"hil",HU:"hu",ISL:"is",IT:"it",JA:"ja",KAN:"kn",KAZ:"kk",KM:"km",KO:"ko",LV:"lv",LT:"lt",MK:"mk",MAL:"ml",MAR:"mr",MN:"mn","SR-ME":"sr-ME",MY:"my",NE:"ne",NO:"no",ORI:"or",FA:"fa",PL:"pl",PT:"pt","PA-IN":"pa-IN",RO:"ro",RU:"ru",SR:"sr",SIN:"si",SK:"sk",SL:"sl","ES-ES":"es-ES",ES:"es",SW:"sw",SV:"sv",TGL:"fil",TA:"ta",TEL:"te",TH:"th",TR:"tr",UK:"uk",UR:"ur",VI:"vi",CY:"cy"};function tt(e,t="en"){const n=A($(e))||t,i=Et[n.toUpperCase()]??n.replace(/_/g,"-");if(!/^[A-Za-z]{2,3}(?:-[A-Za-z0-9]{2,8})*$/.test(i))return t;try{return Intl.getCanonicalLocales(i)[0]??t}catch{return t}}function nt(e,t,n,i,r,s){if(!r.length)return s.unsupported.push({code:"no-supported-items",title:"No supported questionnaire items were found",detail:"This release needs at least one required, ordered single-choice rating item."}),null;if(r.length>G)return s.unsupported.push({code:"too-many-items",title:"The questionnaire has too many items for a participant link",detail:`${r.length} supported items were found; this release accepts at most ${G}.`}),null;const a=r[0].values;if(!se(a))return s.unsupported.push({code:"unsupported-response-values",title:"Response values are not one increasing whole-number scale",detail:"Values must be unique whole numbers from 0 to 100 with one constant positive step."}),null;const v=a.join("|");return r.some(f=>f.values.join("|")!==v)?(s.unsupported.push({code:"mixed-response-scales",title:"Items use different response values",detail:"The current participant interface requires every imported item to share the same ordered numeric scale."}),null):(I(s,{code:"review-scoring",title:"Scoring and reverse scoring require confirmation",detail:"Answer recodes do not establish a questionnaire score. Choose reviewed mean or sum, then mark any reverse-scored items."}),I(s,{code:"review-scale-type",title:"The scale description requires confirmation",detail:"Confirm whether the imported scale should be described as agreement or semantic differential."}),{language:i,name:e.slice(0,120),shortName:Tt(e),version:"1.0.0",description:`Questionnaire imported from ${t}.`,introPrompt:n.slice(0,400),sourceLabel:t,sourceUrl:"",scaleType:qt(r),minimum:a[0],maximum:a.at(-1),step:a[1]-a[0],scoreName:"Questionnaire score",aggregation:"mean",items:r.map(f=>ge({name:f.name.slice(0,120),prompt:f.prompt.slice(0,1e3),lowAnchor:f.labels[0].slice(0,80),highAnchor:f.labels.at(-1).slice(0,80),responseLabels:Object.fromEntries(f.values.map((u,p)=>[String(u),f.labels[p].slice(0,120)]))}))})}function oe(e){return{code:`item-${e.sourceId}`,title:`${e.name}: imported`,detail:`${e.sourceId}; “${e.prompt}”; ordered choices: `+e.values.map((t,n)=>`${t} = ${e.labels[n]}`).join("; ")}}function fe(e,t,n,i,r,s,a={}){return{source:e,sourceName:t,fileName:n,title:i,draft:r,imported:s.imported,confirmations:s.confirmations,unsupported:s.unsupported,canConvert:!!r&&s.unsupported.length===0,...a}}function it(){return{imported:[],confirmations:[],unsupported:[],confirmationCodes:new Set}}function Be(e,t){const n=E(e);if(!n)throw new Error(t);return n}function xt(e,t){const n=e.find(c=>c.Element==="BL"),i=Array.isArray(n?.Payload)?n.Payload.map(E).filter(c=>!!c):[],r=i.filter(c=>c.Type!=="Trash"),s=new Set(i.filter(c=>c.Type==="Trash").flatMap(c=>Array.isArray(c.BlockElements)?c.BlockElements:[]).map(E).filter(c=>!!c).map(c=>$(c.QuestionID)).filter(Boolean));s.size&&I(t,{code:"qualtrics-trash-ignored",title:"Questions in the Qualtrics Trash block were ignored",detail:`${s.size} deleted question${s.size===1?"":"s"} will not appear in the converted questionnaire.`});const a=e.find(c=>c.Element==="FL"),v=E(a?.Payload),f=Array.isArray(v?.Flow)?v.Flow.map(E).filter(c=>!!c):[];if(r.length!==1||f.length!==1||f[0].Type!=="Block")return t.unsupported.push({code:"qualtrics-complex-flow",title:"Qualtrics flow is outside the supported single-block subset",detail:"Use one ordinary question block only. Branches, randomisers, embedded-data flow and multiple blocks are not removed or flattened."}),{order:[],trashQuestionIds:s};const u=$(f[0].ID),p=r.find(c=>$(c.ID)===u);if(!p)return t.unsupported.push({code:"qualtrics-missing-block",title:"The active Qualtrics block could not be resolved",detail:`Survey Flow references ${u||"an unknown block"}.`}),{order:[],trashQuestionIds:s};const d=Array.isArray(p.BlockElements)?p.BlockElements:[],S=[];for(const c of d){const Q=E(c);if(!Q||Q.Type!=="Question"||!$(Q.QuestionID)){t.unsupported.push({code:"qualtrics-non-question-block-item",title:"The Qualtrics block contains unsupported content",detail:"Only ordered question entries are supported in the imported block."});continue}(Q.SkipLogic||Q.DisplayLogic)&&t.unsupported.push({code:"qualtrics-question-logic",title:`${$(Q.QuestionID)} uses question logic`,detail:"Skip and display logic are not imported."}),S.push($(Q.QuestionID))}return{order:S,trashQuestionIds:s}}function Rt(e){return["QuestionJS","JavaScript","DisplayLogic","SkipLogic","ChoiceDisplayLogic","CarryForward","Randomization","ChoiceRandomization"].some(t=>e[t]!==void 0&&e[t]!==null)}function We(e,t,n,i,r){const s=E(e[t]);if(!s)return r.unsupported.push({code:"qualtrics-missing-scale",title:`${i} has no answer scale`,detail:`${t} must contain every visible answer option.`}),null;const a=Xe(e[n],s);if(!a)return r.unsupported.push({code:"qualtrics-unknown-answer-order",title:`${i} has no reliable answer order`,detail:`${n} must list every ${t.toLowerCase()} entry exactly once.`}),null;const v=E(e.RecodeValues);let f;if(v){const p=a.map(d=>ne(v[d]));if(p.some(d=>d===null))return r.unsupported.push({code:"qualtrics-nonnumeric-recodes",title:`${i} has missing or non-integer recode values`,detail:"Every visible option needs an explicit whole-number recode."}),null;f=p}else{if(!a.every((d,S)=>d===String(S+1)))return r.unsupported.push({code:"qualtrics-missing-scale",title:`${i} has no explicit recode table or default sequential choice IDs`,detail:"Without RecodeValues, ChoiceOrder must explicitly list the default IDs 1 through N. The importer will not infer values from other object keys."}),null;f=a.map((d,S)=>S+1),I(r,{code:"qualtrics-default-recodes",title:"Qualtrics default sequential recodes were used",detail:"The QSF omits RecodeValues and explicitly orders default choice IDs 1 through N. Confirm these values against the source survey before conversion."})}const u=a.map((p,d)=>et(s[p],`Answer ${d+1}`,`${i}/${p}`,r));return u.some(p=>p===null)?null:{values:f,labels:u}}function Qt(e,t){let n;try{n=JSON.parse(e)}catch{throw new Error("The Qualtrics QSF file is not valid JSON.")}const i=Be(n,"The Qualtrics QSF root must be a JSON object."),r=Be(i.SurveyEntry,"The file does not contain a Qualtrics SurveyEntry.");if(!Array.isArray(i.SurveyElements))throw new Error("The file does not contain Qualtrics SurveyElements.");const s=it(),a=A($(r.SurveyName))||"Imported Qualtrics questionnaire",v=tt(r.SurveyLanguage,"en"),f=i.SurveyElements.map(E).filter(y=>!!y),{order:u,trashQuestionIds:p}=xt(f,s),d=new Set(f.map(y=>$(y.Element)).filter(y=>y&&!["BL","FL","SQ"].includes(y)));d.size&&I(s,{code:"qualtrics-platform-settings-not-imported",title:"Qualtrics platform and presentation settings are not imported",detail:`The following non-question element types remain in Qualtrics only: ${[...d].sort().join(", ")}. Review the converted participant presentation.`});const S=new Map(f.filter(y=>y.Element==="SQ").map(y=>[$(y.PrimaryAttribute),E(y.Payload)]));if(u.length){const y=new Set(u);for(const _ of S.keys())!_||y.has(_)||p.has(_)||s.unsupported.push({code:"qualtrics-unreferenced-question",title:`${_} is outside the imported active block`,detail:"The importer will not silently omit an active question that is not represented by the supported single-block flow."})}const c=[];for(const y of u){const _=S.get(y);if(!_){s.unsupported.push({code:"qualtrics-missing-question",title:`${y} is missing`,detail:"The active block references a question that is not present in SurveyElements."});continue}if(Rt(_)){s.unsupported.push({code:"qualtrics-unsupported-behaviour",title:`${y} contains logic, randomisation or code`,detail:"The importer does not execute or remove question behaviour."});continue}const W=E(_.Validation);if(E(W?.Settings)?.ForceResponse!=="ON"){s.unsupported.push({code:"qualtrics-optional-question",title:`${y} is not a forced-response question`,detail:"The participant platform currently requires every imported item."});continue}const H=B(_.QuestionText,"Question text",y,s);if(!H)continue;const J=$(_.QuestionType),F=$(_.Selector),q=$(_.SubSelector),ae=A($(_.DataExportTag))||y;if(J==="MC"&&F==="SAVR"){const k=We(_,"Choices","ChoiceOrder",y,s);if(!k)continue;const L={sourceId:y,name:ae,prompt:H,...k};c.push(L),s.imported.push(oe(L));continue}if(J==="Matrix"&&F==="Likert"&&q==="SingleAnswer"){const k=We(_,"Answers","AnswerOrder",y,s),L=E(_.Choices),le=L?Xe(_.ChoiceOrder,L):null;if(!k||!L||!le){le||s.unsupported.push({code:"qualtrics-unknown-row-order",title:`${y} has no reliable matrix row order`,detail:"ChoiceOrder must list every matrix row exactly once."});continue}I(s,{code:"qualtrics-matrix-expanded",title:"Single-answer matrix rows were expanded into items",detail:"Review each generated item against the original matrix before conversion."}),le.forEach((V,Y)=>{const D=et(L[V],`Matrix row ${Y+1}`,`${y}/${V}`,s);if(!D)return;const O={sourceId:`${y}/${V}`,name:D,prompt:`${H} — ${D}`,...k};c.push(O),s.imported.push(oe(O))});continue}s.unsupported.push({code:"qualtrics-unsupported-question",title:`${y} uses an unsupported Qualtrics question type`,detail:`${J||"Unknown"} / ${F||"Unknown"} / ${q||"none"} is not converted. Supported: MC/SAVR and Matrix/Likert/SingleAnswer.`})}const ie=($(r.SurveyDescription).trim()?B(r.SurveyDescription,"Survey description","SurveyEntry",s):null)??"Answer each imported item about the task that you have just completed.",ye=nt(a,"Imported from Qualtrics QSF",ie,v,c,s);return fe("qualtrics-qsf","Qualtrics QSF",t,a,ye,s)}function R(e,t){const n=[...e.documentElement.children].find(r=>r.localName===t);if(!n)return[];const i=[...n.children].find(r=>r.localName==="rows");return i?[...i.children].filter(r=>r.localName==="row").map(r=>Object.fromEntries([...r.children].map(s=>[s.localName,s.textContent??""]))):[]}function z(e,t,n,i,r){return e.find(s=>s[t]===n&&(!s.language||s.language===r))?.[i]??""}const Pt={answer_width:["0"],answer_order:["normal"],array_filter_style:["0"],clear_default:["N"],dropdown_prefix:["0"],hidden:["0"],hide_tip:["0"],max_answers:["1"],min_answers:["1"],other_comment_mandatory:["0"],other_numbers_only:["0"],other_position:["default"],page_break:["0"],public_statistics:["0"],random_order:["0"],save_as_default:["N"],scale_export:["0"],slider_rating:["0"],statistics_graphtype:["0"],statistics_showgraph:["1"],time_limit_action:["1"],time_limit_disable_next:["0"],time_limit_disable_prev:["0"],use_dropdown:["0"]};function kt(e,t){if(!e.length)return;const n=e.filter(i=>{const r=i.attribute?.trim()??"",s=i.value?.trim()??"";return s?!Pt[r]?.includes(s):!1});if(n.length){const i=n.slice(0,6).map(r=>`${r.qid||"unknown question"}: ${r.attribute||"unknown"}=${r.value||""}`).join("; ");t.unsupported.push({code:"limesurvey-question-attributes",title:"The LimeSurvey export contains active or unknown question attributes",detail:`${i}${n.length>6?`; and ${n.length-6} more`:""}. These settings may change validation, randomisation, timing or presentation and are not discarded.`});return}I(t,{code:"limesurvey-default-question-attributes",title:"Default LimeSurvey question settings were not imported",detail:"Only empty or known default attributes were present; no active validation, randomisation or timing rule was found. Review the converted participant presentation."})}function He(e){if(!e.trim())return"";const n=new DOMParser().parseFromString(`<body>${e}</body>`,"text/html");return A(n.body.textContent??"")}function rt(e){const t=e.map(r=>ne(r.sortorder));if(t.some(r=>r===null))return!1;const n=t;if(new Set(n).size!==n.length)return!1;const i=n[0];return(i===0||i===1)&&n.every((r,s)=>r===i+s)}function ot(e){return e.every((t,n)=>t.code===`A${String(n+1).padStart(3,"0")}`)}function st(e,t){if(e.type==="5")return[1,2,3,4,5];if(e.type!=="F"&&e.type!=="L"&&e.type!=="!")return null;const n=t.filter(s=>s.qid===e.qid&&(!s.scale_id||s.scale_id==="0")).sort((s,a)=>(Number(s.sortorder)||0)-(Number(a.sortorder)||0));if(!rt(n))return null;const i=n.map(s=>ne(s.code));if(i.every(s=>s!==null)&&se(i))return i;const r=n.map(s=>ne(s.assessment_value));return r.every(s=>s!==null)&&se(r)?r:ot(n)?n.map((s,a)=>a+1):null}function Lt(e,t,n){const i=new Map;for(const r of e){if(r.mandatory!=="Y"||r.other&&r.other!=="N"||r.relevance&&r.relevance!=="1")continue;const s=st(r,n);if(!s)continue;const a=r.type==="F"?t.filter(u=>u.parent_qid===r.qid).length:1;if(a<1)continue;const v=s.join("|"),f=i.get(v)??{questions:[],values:s,itemCount:0};f.questions.push(r),f.itemCount+=a,i.set(v,f)}return[...i.entries()].map(([r,s])=>({id:`values-${r.replace(/\|/g,"-")}`,name:s.questions.map(a=>a.title||a.qid).join(" + "),sourceQuestionCount:s.questions.length,itemCount:s.itemCount,responseValues:s.values,questionTypes:[...new Set(s.questions.map(a=>a.type||"unknown"))]}))}function Je(e,t,n,i,r){const s=e.qid||e.title||"unknown-question",a=t.filter(d=>d.qid===e.qid&&(!d.scale_id||d.scale_id==="0")).sort((d,S)=>(Number(d.sortorder)||0)-(Number(S.sortorder)||0));if(!rt(a))return r.unsupported.push({code:"limesurvey-answer-order",title:`${e.title||s} has no reliable answer order`,detail:"Every answer needs one unique consecutive sort order starting at 0 or 1."}),null;const v=a.map(d=>ne(d.code)),f=a.map(d=>ne(d.assessment_value));let u;if(v.every(d=>d!==null)&&se(v))u=v;else if(f.every(d=>d!==null)&&se(f))u=f,I(r,{code:"limesurvey-assessment-values",title:"LimeSurvey assessment values were used as response values",detail:"Verify every imported value against the LimeSurvey answer table."});else if(ot(a))u=a.map((d,S)=>S+1),I(r,{code:"limesurvey-positional-values",title:"LimeSurvey default answer codes were converted to ordered positions",detail:"The source uses A001 through A00N rather than numeric scores. The converted scale uses positions 1 through N. Confirm the intended values and scoring before conversion."});else return r.unsupported.push({code:"limesurvey-unsupported-values",title:`${e.title||s} has no safe increasing numeric recode`,detail:"Answer codes or assessment values must form one increasing whole-number scale with a constant step."}),null;const p=a.map((d,S)=>{const c=z(n,"aid",d.aid,"answer",i)||d.answer;return A(c??"")?B(c,`Answer ${S+1}`,`${e.title||s}/${d.code}`,r):(I(r,{code:"limesurvey-blank-scale-labels",title:"Blank scale positions will be shown as their numeric values",detail:"LimeSurvey leaves one or more intermediate labels blank. The converted questionnaire shows the reviewed numeric response value at those positions."}),String(u[S]))});return p.some(d=>!d)?null:{values:u,labels:p}}function Nt(e,t,n,i){if(/<!DOCTYPE|<!ENTITY|<\?xml-stylesheet/i.test(e))throw new Error("The LimeSurvey file contains a DTD, entity or stylesheet declaration and was not parsed.");const s=new DOMParser().parseFromString(e,"application/xml");if(s.querySelector("parsererror"))throw new Error("The LimeSurvey file is not valid XML.");const a=s.querySelector("LimeSurveyDocType")?.textContent?.trim();if(a!=="Survey"&&a!=="Group"&&a!=="Question")throw new Error("The XML file is not a LimeSurvey survey, question-group or question export.");const v=a==="Group"?"limesurvey-lsg":a==="Question"?"limesurvey-lsq":"limesurvey-lss",f=a==="Group"?"LimeSurvey LSG":a==="Question"?"LimeSurvey LSQ":"LimeSurvey LSS",u=it(),p=[...s.querySelectorAll("languages > language")].map(o=>o.textContent?.trim()??"").filter(Boolean),d=R(s,"surveys"),c=A(d[0]?.language??"")||p[0]||"";!c||p.length&&!p.includes(c)?u.unsupported.push({code:"limesurvey-base-language",title:"The LimeSurvey base language could not be resolved",detail:"The survey language must match one language declared in the LSS export."}):p.length>1&&I(u,{code:"limesurvey-base-language-only",title:`Only the LimeSurvey base language (${c}) will be imported`,detail:`Additional languages (${p.filter(o=>o!==c).join(", ")}) remain in LimeSurvey and are not included in the converted definition. Confirm the intended participant language.`});const Q=R(s,"surveys_languagesettings"),ie=Q.find(o=>!o.surveyls_language||o.surveyls_language===c)??Q[0],ye=A(ie?.surveyls_title??"")||"Imported LimeSurvey questionnaire",y=R(s,"questions").filter(o=>!o.language||!c||o.language===c),_=y.find(o=>!o.parent_qid||o.parent_qid==="0"),W=_?.gid||"__lsq__",N=a==="Question"?[{gid:W,group_order:"0",grelevance:"1",randomization_group:""}]:R(s,"groups"),H=a==="Question"?[{gid:W,group_name:_?.title||"Imported LimeSurvey question",description:"",language:c}]:R(s,"group_l10ns"),J=y.map(o=>({...o,gid:o.gid||W}));if(!N.length)throw new Error("The LimeSurvey export contains no questionnaire group.");const F=N.slice().sort((o,b)=>(Number(o.group_order)||0)-(Number(b.group_order)||0)).map(o=>{const b=H.find(T=>T.gid===o.gid&&(!T.language||T.language===c)),re=J.filter(T=>T.gid===o.gid&&(!T.parent_qid||T.parent_qid==="0"));return{id:o.gid,name:A(b?.group_name??o.group_name??"")||`Group ${o.gid}`,questionCount:re.length,questionTypes:[...new Set(re.map(T=>T.type||"unknown"))]}});if(a==="Survey"&&N.length>1&&!n)return fe(v,f,t,ye,null,u,{requiresGroupSelection:!0,groupOptions:F});const q=n?N.find(o=>o.gid===n):N[0];if(!q)throw new Error("Choose a questionnaire group contained in this LimeSurvey export.");const ae=H.find(o=>o.gid===q.gid&&(!o.language||o.language===c)),k=A(ae?.group_name??q.group_name??"")||`Group ${q.gid}`,L=J.filter(o=>o.gid===q.gid),V=R(s,"subquestions").filter(o=>!o.language||!c||o.language===c).map(o=>({...o,gid:o.gid||W})).filter(o=>o.gid===q.gid||L.some(b=>b.qid===o.parent_qid)),Y=L.filter(o=>!o.parent_qid||o.parent_qid==="0").sort((o,b)=>(Number(o.question_order)||0)-(Number(b.question_order)||0)),D=R(s,"answers").filter(o=>!o.language||!c||o.language===c),O=Lt(Y,V,D);if(a==="Survey"&&N.length>1&&I(u,{code:"limesurvey-selected-group-only",title:`Only “${k}” will be converted`,detail:`${N.length-1} other survey group${N.length===2?"":"s"} will remain outside the converted questionnaire. The researcher selected this group explicitly; no other group is silently flattened or removed.`}),a==="Question"&&I(u,{code:"limesurvey-question-context-not-retained",title:"The LimeSurvey question will run as a standalone questionnaire",detail:"An LSQ contains one question and its local answer settings, but not its original survey or group context. Confirm that standalone use is intended."}),q.grelevance&&q.grelevance!=="1"&&I(u,{code:"limesurvey-group-relevance-not-retained",title:"The source group display condition will not be retained",detail:`The LimeSurvey group condition is “${A(q.grelevance)}”. The converted questionnaire runs this selected group as a standalone instrument. Confirm that this is intended.`}),A(q.randomization_group??"")&&I(u,{code:"limesurvey-group-randomisation-not-retained",title:"The source group randomisation setting will not be retained",detail:"The converted questionnaire uses the reviewed exported question order. Confirm that a fixed order is suitable."}),O.length>1&&!i)return fe(v,f,t,k,null,u,{groupOptions:F,selectedGroupId:q.gid,requiresRatingSetSelection:!0,ratingSetOptions:O});const K=i?O.find(o=>o.id===i):O[0];if(i&&!K)throw new Error("Choose a rating set contained in the selected LimeSurvey group.");const Te=K?.responseValues.join("|"),ve=Te?Y.filter(o=>st(o,D)?.join("|")===Te&&o.mandatory==="Y"&&(!o.other||o.other==="N")&&(!o.relevance||o.relevance==="1")):Y,Ee=new Set(ve.map(o=>o.qid)),xe=V.filter(o=>Ee.has(o.parent_qid)),ue=new Set(ve.map(o=>o.qid));for(const o of xe)ue.add(o.qid);const ce=Y.filter(o=>!Ee.has(o.qid));K&&ce.length&&I(u,{code:"limesurvey-selected-rating-set-only",title:`Only the ${K.responseValues[0]}–${K.responseValues.at(-1)} rating set will be converted`,detail:`${ce.length} other source question${ce.length===1?"":"s"} (${ce.map(o=>`${o.title||o.qid} [type ${o.type||"unknown"}]`).join(", ")}) will remain in LimeSurvey. They are listed here rather than silently removed or mixed into an invalid score.`}),R(s,"conditions").filter(o=>ue.has(o.qid)||ue.has(o.cqid)).length&&u.unsupported.push({code:"limesurvey-conditions",title:"The selected LimeSurvey group contains question conditions",detail:"Branching and conditional relevance are not imported."});for(const[o,b]of[["assessments","assessment rules"],["defaultvalues","default answers"],["quotas","quota rules"],["quota_members","quota membership rules"],["quota_languages","localised quota messages"]])R(s,o).length&&u.unsupported.push({code:`limesurvey-${o.replace(/_/g,"-")}`,title:`The LimeSurvey export contains ${b}`,detail:`${b[0].toUpperCase()}${b.slice(1)} are not executed or silently discarded.`});kt(R(s,"question_attributes").filter(o=>ue.has(o.qid)),u);const Z=R(s,"question_l10ns"),Re=R(s,"answer_l10ns"),M=[];I(u,{code:"limesurvey-platform-settings-not-imported",title:"LimeSurvey platform and presentation settings are not imported",detail:"Theme, navigation, notification and publication settings remain in LimeSurvey. Review the converted participant presentation."});for(const o of ve){const b=o.qid||o.title||"unknown-question";if(o.mandatory!=="Y"){u.unsupported.push({code:"limesurvey-optional-question",title:`${o.title||b} is not mandatory`,detail:"The participant platform currently requires every imported item."});continue}if(o.other&&o.other!=="N"){u.unsupported.push({code:"limesurvey-other-answer",title:`${o.title||b} allows an “Other” answer`,detail:"Free-text “Other” responses are not imported."});continue}if(o.relevance&&o.relevance!=="1"){u.unsupported.push({code:"limesurvey-question-relevance",title:`${o.title||b} uses relevance logic`,detail:"Conditional questions are not imported."});continue}const re=z(Z,"qid",o.qid,"question",c)||o.question,T=z(Z,"qid",o.qid,"help",c)||o.help,ut=z(Z,"qid",o.qid,"script",c)||o.script;if(A(ut||"")){u.unsupported.push({code:"limesurvey-question-script",title:`${o.title||b} contains a question script`,detail:"Imported scripts are never executed or silently removed."});continue}if(Ce.test(T||"")||qe.test(T||"")||$e.test(T||"")){u.unsupported.push({code:"limesurvey-question-help-structure",title:`${o.title||b} contains dynamic or structured help content`,detail:"Executable code, expressions, media and interactive help are not imported."});continue}if(He(T||"")){u.unsupported.push({code:"limesurvey-question-help",title:`${o.title||b} contains participant help text`,detail:"Question help text is not silently merged into or removed from the item wording."});continue}const j=A(re??"")?B(re,"Question text",o.title||b,u):null;if(o.type==="5"){if(!j){u.unsupported.push({code:"missing-visible-text",title:"Question text is empty",detail:`${o.title||b} does not contain participant-visible text.`});continue}const P={sourceId:o.title||b,name:o.title||`Item ${M.length+1}`,prompt:j,values:[1,2,3,4,5],labels:["1","2","3","4","5"]};M.push(P),u.imported.push(oe(P))}else if(o.type==="L"||o.type==="!"){if(!j){u.unsupported.push({code:"missing-visible-text",title:"Question text is empty",detail:`${o.title||b} does not contain participant-visible text.`});continue}const P=Je(o,D,Re,c,u);if(!P)continue;const X={sourceId:o.title||b,name:o.title||`Item ${M.length+1}`,prompt:j,...P};M.push(X),u.imported.push(oe(X))}else if(o.type==="F"){const P=xe.filter(w=>w.parent_qid===o.qid).sort((w,ee)=>(Number(w.question_order)||0)-(Number(ee.question_order)||0));if(!P.length){u.unsupported.push({code:"limesurvey-array-without-rows",title:`${o.title||b} has no array row`,detail:"A LimeSurvey Array question must contain at least one explicit row before it can be converted."});continue}if(P.some(w=>w.relevance&&w.relevance!=="1"||w.other&&w.other!=="N"||w.scale_id&&w.scale_id!=="0")){u.unsupported.push({code:"limesurvey-array-row-behaviour",title:`${o.title||b} contains an unsupported array row`,detail:"Conditional, “Other” or secondary-scale array rows are not flattened."});continue}const X=Je(o,D,Re,c,u);if(!X)continue;P.length===1?I(u,{code:"limesurvey-single-row-array-expanded",title:"Single-row LimeSurvey Array questions were converted to rating items",detail:"Each source question contains one array row. Review the displayed item wording and response scale against LimeSurvey."}):I(u,{code:"limesurvey-array-expanded",title:"LimeSurvey Array rows were converted to separate rating items",detail:"The source array heading is combined with each visible row label when both contain text. Review the converted wording and order."});for(const w of P){const ee=w.qid||w.title||`${b}-row`,Pe=z(Z,"qid",w.qid,"question",c)||w.question,de=z(Z,"qid",w.qid,"help",c)||w.help,ct=z(Z,"qid",w.qid,"script",c)||w.script;if(A(ct||"")||Ce.test(de||"")||qe.test(de||"")||$e.test(de||"")||He(de||"")){u.unsupported.push({code:"limesurvey-array-row-content",title:`${w.title||ee} contains unsupported row content`,detail:"Array-row scripts, dynamic content and help text are not imported."});continue}const pe=A(Pe??"")?B(Pe,"Array row text",w.title||ee,u):null,ke=j&&pe?`${j}: ${pe}`:j||pe;if(!ke){u.unsupported.push({code:"missing-visible-text",title:"Array item text is empty",detail:`${w.title||ee} and its parent question contain no participant-visible text.`});continue}const Le={sourceId:`${o.title||b}/${w.title||ee}`,name:P.length===1&&!pe?o.title||`Item ${M.length+1}`:w.title||o.title||`Item ${M.length+1}`,prompt:ke,values:[...X.values],labels:[...X.labels]};M.push(Le),u.imported.push(oe(Le))}}else u.unsupported.push({code:"limesurvey-unsupported-question",title:`${o.title||b} uses unsupported LimeSurvey type ${o.type||"unknown"}`,detail:"Supported in this release: List (Radio), 5 Point Choice and reviewed Array rating rows."})}const Qe=a==="Question"?"":ae?.description||q.description||ie?.surveyls_description||ie?.surveyls_welcometext||"",at=A(Qe)?B(Qe,"Survey introduction","surveys_languagesettings",u):null,lt=nt(k,`Imported from ${f}`,at??"Answer each imported item about the task that you have just completed.",tt(c,"en"),M,u);return fe(v,f,t,k,lt,u,{groupOptions:F,selectedGroupId:q.gid,ratingSetOptions:O,selectedRatingSetId:K?.id})}function Dt(e,t){const n=t.toLowerCase().split(".").at(-1),i=e.trimStart();if(i.startsWith("{")&&/"SurveyElements"/.test(e))return"qualtrics-qsf";if(i.startsWith("<")&&/<LimeSurveyDocType>Survey</.test(e))return"limesurvey-lss";if(i.startsWith("<")&&/<LimeSurveyDocType>Group</.test(e))return"limesurvey-lsg";if(i.startsWith("<")&&/<LimeSurveyDocType>Question</.test(e))return"limesurvey-lsq";if(n==="qsf")return"qualtrics-qsf";if(n==="lss")return"limesurvey-lss";if(n==="lsg")return"limesurvey-lsg";if(n==="lsq")return"limesurvey-lsq";throw n==="lsa"?new Error("LimeSurvey LSA archives are not imported because they may contain responses, tokens and participant data. Export survey structure as LSS instead."):n==="lsl"?new Error("A LimeSurvey LSL file contains only a reusable label set, not a questionnaire. Export the containing question as LSQ, group as LSG or survey as LSS."):n==="csv"||n==="tsv"||n==="xls"||n==="xlsx"?new Error("CSV, TSV and spreadsheet files are ambiguous table formats used for response data or platform-specific bulk authoring. Import the file into its source platform, review it there, then export Qualtrics QSF or LimeSurvey LSS, LSG or LSQ."):n==="sav"||n==="spss"||n==="vv"?new Error("This is a response-data format, not a reusable questionnaire definition. Export Qualtrics QSF or LimeSurvey LSS, LSG or LSQ from the source questionnaire."):n==="txt"||n==="doc"||n==="docx"||n==="rtf"||n==="odt"?new Error("Text and word-processing files may be manually prepared authoring files or readable exports; they are not an unambiguous native structure export. Import the file into Qualtrics or LimeSurvey first, review it there, then export QSF, LSS, LSG or LSQ."):n==="pdf"||n==="html"||n==="htm"||n==="xml"||n==="zip"?new Error("This print, generic XML or archive format is not an unambiguous native questionnaire structure export. Export Qualtrics QSF or LimeSurvey LSS, LSG or LSQ."):n==="json"?new Error("This JSON file is not a Qualtrics QSF. If it is an AQP questionnaire definition, use the separate “Reuse an AQP questionnaire definition” route; otherwise export Qualtrics QSF."):new Error("Choose a Qualtrics .qsf file or a LimeSurvey .lss, .lsg or .lsq file.")}function Se(e,t,n="auto",i,r){const s=new TextEncoder().encode(e).length;if(!e.trim())throw new Error("The selected questionnaire export is empty.");if(s>$t)throw new Error("The selected questionnaire export is larger than the 2 MB review limit.");const a=Dt(e,t);if(n!=="auto"&&n!==a){const v=a==="qualtrics-qsf"?"Qualtrics QSF":a==="limesurvey-lss"?"LimeSurvey LSS":a==="limesurvey-lsg"?"LimeSurvey LSG":"LimeSurvey LSQ";throw new Error(`The selected file looks like ${v}, not the chosen format.`)}return a==="qualtrics-qsf"?Qt(e,t):Nt(e,t,i,r)}var Ot=Object.defineProperty,Mt=Object.getOwnPropertyDescriptor,h=(e,t,n,i)=>{for(var r=i>1?void 0:i?Mt(t,n):t,s=e.length-1,a;s>=0;s--)(a=e[s])&&(r=(i?a(t,n,r):a(r))||r);return i&&r&&Ot(t,n,r),r};const Ft=Ie.trim().split(/\r?\n/).filter(Boolean).length,te=Ae.match(/var bridgeBuild = '([^']+)'/)?.[1]??"unidentified",Ve=[{key:"source",title:"Choose the questionnaire source"},{key:"scoring",title:"Confirm the questionnaire and scoring"},{key:"study",title:"Enter study details"},{key:"support",title:"Set participant support"},{key:"collection",title:"Choose result collection"},{key:"review",title:"Review and generate"}],Ye=[{key:"source",title:"Choose the questionnaire source"},{key:"upload",title:"Upload the file and choose the relevant part"},{key:"questions",title:"Review the questions"},{key:"answers",title:"Review answer choices and stored values"},{key:"warnings",title:"Resolve import warnings"},{key:"scoring",title:"Confirm the scoring rule"},{key:"study",title:"Enter study details"},{key:"support",title:"Set participant support"},{key:"collection",title:"Choose result collection"},{key:"review",title:"Review and generate"}],_e="accessible-questionnaire-v0.8-conductor-draft";function jt(e){const t=Array.isArray(e)?e:[e];return t.length>0&&t.some(n=>{if(!n||typeof n!="object")return!1;const i=n;return"study"in i&&"responses"in i&&"result"in i})}function Ut(e){const t="PASTE_THE_GENERATED_PARTICIPANT_PAGE_URL_HERE";if(!e||e.includes(t))throw new Error("A generated participant URL is required for the Qualtrics question HTML.");const n=e.replace(/&/g,"&amp;").replace(/"/g,"&quot;");return _t.trim().replace(t,n)}function zt(e){const t=e?["Questionnaire:","${e://Field/__js_AQP_INSTRUMENT_NAME}","","${e://Field/__js_AQP_SCORE_NAME}:","${e://Field/__js_AQP_PRIMARY_SCORE}"].join(`
`):"";return St.replace("{{OPTIONAL_SCORE_BLOCK}}",t).replace(/\n{3,}/g,`

`).trim()}let m=class extends ht{constructor(){super(...arguments),this.setupRoute="ready-made",this.wizardStepIndex=0,this.scoringConfirmed=!1,this.importWarningsAcknowledged=!1,this.instrumentId=ft,this.customDefinition=null,this.customDraft=ze(),this.customBuilderOpen=!1,this.platformImportSource="auto",this.platformImportReview=null,this.platformImportConfirmed=!1,this.platformImportSelectedGroupId="",this.platformImportSelectedRatingSetId="",this.platformImportContents="",this.platformImportFileName="",this.studyId="",this.studyTitle="",this.taskLabel="",this.participantCode="",this.showScoreToParticipant=!1,this.showSimpleLanguage=!1,this.answerMode="standard",this.largeText=!1,this.audioGuidance=!1,this.recoveryEnabled=!0,this.participantAdjustmentPolicy="participant-choice",this.voiceInputAvailable=!0,this.gazeInputAvailable=!1,this.collectionMode="local",this.qualtricsSurveyUrl="",this.generatedConfig=null,this.participantUrl="",this.message="",this.definitionConfirmation="",this.configurationConfirmation="",this.errorMessage="",this.completedResults=[],this.draftRestored=!1,this.handleWizardPopState=e=>{const t=Number(e.state?.aqpConductorStep);!Number.isInteger(t)||t<0||t>=this.wizardSteps.length||(this.wizardStepIndex=t,this.errorMessage="",this.focusWizardHeading())},this.continueWizard=()=>{this.errorMessage="";const e=this.wizardStep.key;try{if(e==="upload"){const n=this.platformImportReview;if(!n)throw new Error("Choose and review a questionnaire export before continuing.");if(n.requiresGroupSelection)throw new Error("Choose the LimeSurvey questionnaire group before continuing.");if(n.requiresRatingSetSelection)throw new Error("Choose the compatible LimeSurvey rating set before continuing.");if(!n.canConvert||!n.draft)throw new Error("This export contains unsupported content. Correct the source and review a new export before continuing.")}if(e==="warnings"&&!this.importWarningsAcknowledged)throw new Error("Acknowledge the listed import findings before continuing.");if(e==="scoring"){if(this.setupRoute==="imported"){if(!this.platformImportConfirmed)throw new Error("Confirm the scoring rule and imported values against the source before continuing.");if(this.usePlatformImport(),this.customDefinition?.id!==this.instrumentId)throw new Error("The reviewed questionnaire could not be activated. Check the scoring fields and try again.")}else if(!this.scoringConfirmed)throw new Error("Confirm that the selected questionnaire and scoring rule match the study protocol.")}e==="study"&&this.validateStudyDetails(),e==="collection"&&this.currentCollectionConfig();const t=Math.min(this.wizardStepIndex+1,this.wizardSteps.length-1);if(t===this.wizardStepIndex)return;this.wizardStepIndex=t,window.history.pushState({...window.history.state??{},aqpConductorStep:t},"",window.location.href),this.message=`Step ${t+1} of ${this.wizardSteps.length}: ${this.wizardSteps[t].title}.`,this.focusWizardHeading()}catch(t){this.showError(t instanceof Error?t.message:"This step is incomplete.")}},this.previousWizard=()=>{if(this.wizardStepIndex===0)return;const e=this.wizardStepIndex-1;this.wizardStepIndex=e,window.history.pushState({...window.history.state??{},aqpConductorStep:e},"",window.location.href),this.errorMessage="",this.message=`Step ${e+1} of ${this.wizardSteps.length}: ${this.wizardSteps[e].title}.`,this.focusWizardHeading()},this.selectInstrument=e=>{const t=e.currentTarget.value,n=this.availableDefinitions.find(i=>i.id===t)??null;n&&(this.instrumentId=t,n.supports.simplerExplanations||(this.showSimpleLanguage=!1),n.supports.smileyLandmarks||(this.answerMode="standard"),this.scoringConfirmed=!1,this.generatedConfig=null,this.participantUrl="",this.definitionConfirmation=`${n.name} ${n.version} selected. Complete the study details, then generate a new configuration.`,this.configurationConfirmation="",this.message=`${n.name} selected. Generate a new configuration before testing.`)},this.importPlatformQuestionnaire=async e=>{const t=e.currentTarget,n=t.files?.[0];if(n){this.errorMessage="",this.definitionConfirmation="",this.platformImportReview=null,this.platformImportConfirmed=!1,this.importWarningsAcknowledged=!1;try{const i=await n.text();this.platformImportContents=i,this.platformImportFileName=n.name,this.platformImportSelectedGroupId="",this.platformImportSelectedRatingSetId="";const r=Se(i,n.name,this.platformImportSource);this.platformImportReview=r,this.platformImportSelectedGroupId=r.selectedGroupId??"",r.draft&&(this.customDraft=structuredClone(r.draft)),this.message=r.requiresGroupSelection?"Choose one LimeSurvey questionnaire group to review.":r.requiresRatingSetSelection?"Choose one compatible LimeSurvey rating set to review.":r.canConvert?"Import review ready. Check every section and confirm scoring before conversion.":"Import review found unsupported content. No partial questionnaire was created.",this.revealConductorResult("#platform-import-review")}catch(i){this.showError(i instanceof Error?i.message:"The questionnaire export could not be reviewed.")}finally{t.value=""}}},this.reviewSelectedLimeSurveyGroup=()=>{if(!(!this.platformImportContents||!this.platformImportFileName||!this.platformImportSelectedGroupId)){this.errorMessage="",this.definitionConfirmation="",this.platformImportConfirmed=!1,this.importWarningsAcknowledged=!1;try{const e=Se(this.platformImportContents,this.platformImportFileName,this.platformImportSource,this.platformImportSelectedGroupId);this.platformImportReview=e,this.platformImportSelectedRatingSetId="",e.draft&&(this.customDraft=structuredClone(e.draft)),this.message=e.requiresRatingSetSelection?"Choose one compatible rating set in the selected LimeSurvey group.":e.canConvert?"Selected group review ready. Check every section and confirm scoring before conversion.":"The selected group contains unsupported content. No partial questionnaire was created.",this.revealConductorResult("#platform-import-review")}catch(e){this.showError(e instanceof Error?e.message:"The selected LimeSurvey group could not be reviewed.")}}},this.reviewSelectedLimeSurveyRatingSet=()=>{if(!(!this.platformImportContents||!this.platformImportFileName||!this.platformImportSelectedGroupId||!this.platformImportSelectedRatingSetId)){this.errorMessage="",this.definitionConfirmation="",this.platformImportConfirmed=!1,this.importWarningsAcknowledged=!1;try{const e=Se(this.platformImportContents,this.platformImportFileName,this.platformImportSource,this.platformImportSelectedGroupId,this.platformImportSelectedRatingSetId);this.platformImportReview=e,e.draft&&(this.customDraft=structuredClone(e.draft)),this.message=e.canConvert?"Selected rating-set review ready. Check every section and confirm scoring before conversion.":"The selected rating set contains unsupported content. No partial questionnaire was created.",this.revealConductorResult("#platform-import-review")}catch(e){this.showError(e instanceof Error?e.message:"The selected LimeSurvey rating set could not be reviewed.")}}},this.usePlatformImport=()=>{const e=this.platformImportReview;if(!(!e?.canConvert||!e.draft||!this.platformImportConfirmed)){this.errorMessage="",this.definitionConfirmation="";try{const t=Ge(this.customDraft);this.activateCustomDefinition(t,"imported")}catch(t){this.showError(t instanceof Error?t.message:"The reviewed questionnaire could not be converted.")}}},this.addCustomItem=()=>{this.customDraft.items.length>=G||(this.customDraft={...this.customDraft,items:[...this.customDraft.items,ge({name:`Item ${this.customDraft.items.length+1}`})]})},this.useCustomDraft=()=>{this.errorMessage="",this.definitionConfirmation="";try{this.activateCustomDefinition(Ge(this.customDraft),"validated")}catch(e){this.showError(e instanceof Error?e.message:"The custom questionnaire could not be validated.")}},this.importCustomDefinition=async e=>{const t=e.currentTarget,n=t.files?.[0];if(n){this.errorMessage="",this.definitionConfirmation="";try{const i=Ze(JSON.parse(await n.text()));this.activateCustomDefinition(i,"imported")}catch(i){this.showError(i instanceof Error?i.message:"The questionnaire definition file could not be read.")}finally{t.value=""}}},this.downloadCustomDefinition=()=>{this.customDefinition&&me(Ct(this.customDefinition),JSON.stringify(this.customDefinition,null,2),"application/json")},this.resetCustomDraft=()=>{this.customDraft=ze(),this.platformImportReview=null,this.platformImportConfirmed=!1,this.importWarningsAcknowledged=!1,this.platformImportSelectedGroupId="",this.platformImportSelectedRatingSetId="",this.platformImportContents="",this.platformImportFileName="",this.message="Custom questionnaire builder fields reset. The selected questionnaire is unchanged until you validate a new definition."},this.updateParticipantCode=e=>{if(this.participantCode=e.currentTarget.value.trim(),!this.generatedConfig)return;const t=U(this.participantCode);this.participantUrl=t?De(new URL("index.html",window.location.href).toString(),this.generatedConfig,this.participantCode):"",this.configurationConfirmation=t?`Participant-specific link ready for code ${this.participantCode}.`:""},this.generateParticipantLink=()=>{this.errorMessage="",this.configurationConfirmation="";try{if(!U(this.participantCode))throw new Error("Enter a pseudonymous participant code using 1–32 letters, numbers, hyphens or underscores.");const e=Oe({instrumentId:this.instrumentId,...this.customDefinition?.id===this.instrumentId?{questionnaireDefinition:this.customDefinition}:{},studyId:this.studyId,studyTitle:this.studyTitle,taskLabel:this.taskLabel,showScoreToParticipant:this.showScoreToParticipant,support:this.currentSupportConfig(),collection:this.currentCollectionConfig()});this.useConfiguration(e),this.configurationConfirmation="Participant link and configuration generated. The ready-to-use files and link are shown below.",this.message="",this.revealConductorResult("#configuration-ready-panel")}catch(e){this.showError(e instanceof Error?e.message:"The study configuration could not be generated.")}},this.copyParticipantLink=async()=>{this.participantUrl&&await this.copySetupAsset(this.participantUrl,"participant link")},this.copySetupAsset=async(e,t)=>{try{if(!navigator.clipboard?.writeText)throw new Error("Clipboard API unavailable.");await navigator.clipboard.writeText(e),this.message=`${t.charAt(0).toUpperCase()}${t.slice(1)} copied.`}catch{this.message=`Automatic copy was unavailable. Select and copy the ${t} from its text box.`}},this.downloadConfiguration=()=>{this.generatedConfig&&me(`${this.generatedConfig.studyId}-${this.generatedConfig.configId}.json`,JSON.stringify(this.generatedConfig,null,2),"application/json")},this.importConfiguration=async e=>{const t=e.currentTarget,n=t.files?.[0];if(n){this.errorMessage="",this.configurationConfirmation="";try{const i=JSON.parse(await n.text()),r=Me(i);if(!r)throw jt(i)?new Error("This is a completed result file, not a study configuration. Import the JSON downloaded from Configuration ready."):new Error("This is not a valid Version 0.8 study configuration or supported Version 0.7 configuration.");this.useConfiguration(r),this.configurationConfirmation=U(this.participantCode)?"Configuration imported and participant-specific link reproduced. The configuration ID and files are shown below.":"Configuration imported. Enter the approved pseudonymous participant code to create a participant-specific link.",this.message="",this.revealConductorResult("#configuration-ready-panel")}catch(i){this.showError(i instanceof Error?i.message:"The configuration file could not be read.")}finally{t.value=""}}},this.refreshResults=()=>{this.completedResults=gt()},this.exportResultsJson=()=>{this.completedResults.length&&me(`accessible-questionnaire-results-${new Date().toISOString().slice(0,10)}.json`,JSON.stringify(this.completedResults,null,2),"application/json")},this.exportResultsCsv=()=>{this.completedResults.length&&me(`accessible-questionnaire-results-${new Date().toISOString().slice(0,10)}.csv`,`\uFEFF${yt(this.completedResults)}`,"text/csv")},this.eraseResults=()=>{window.confirm("Erase every completed questionnaire record stored by this site in this browser? Confirm only after checking the exported files.")&&(vt(),this.refreshResults(),this.message="Local completed records erased.")}}connectedCallback(){super.connectedCallback(),this.restoreConductorDraft(),this.refreshResults(),window.addEventListener("storage",this.refreshResults),window.addEventListener("popstate",this.handleWizardPopState),window.history.replaceState({...window.history.state??{},aqpConductorStep:this.wizardStepIndex},"",window.location.href)}disconnectedCallback(){window.removeEventListener("storage",this.refreshResults),window.removeEventListener("popstate",this.handleWizardPopState),super.disconnectedCallback()}updated(){this.draftRestored&&this.persistConductorDraft()}createRenderRoot(){return this}get definition(){return bt(this.instrumentId,this.customDefinition??void 0)}get availableDefinitions(){return this.customDefinition?[...Fe,this.customDefinition]:Fe}get wizardSteps(){return this.setupRoute==="imported"?Ye:Ve}get wizardStep(){return this.wizardSteps[Math.min(this.wizardStepIndex,this.wizardSteps.length-1)]}selectSetupRoute(e){this.setupRoute!==e&&(this.setupRoute=e,this.wizardStepIndex=0,this.errorMessage="",this.scoringConfirmed=!1,this.importWarningsAcknowledged=!1,this.generatedConfig=null,this.participantUrl="",this.replaceWizardHistory())}replaceWizardHistory(){window.history.replaceState({...window.history.state??{},aqpConductorStep:this.wizardStepIndex},"",window.location.href)}focusWizardHeading(){this.updateComplete.then(()=>{const e=this.querySelector("#conductor-step-heading");e&&be(e,{block:"start"})})}persistConductorDraft(){try{window.sessionStorage.setItem(_e,JSON.stringify({setupRoute:this.setupRoute,wizardStepIndex:this.wizardStepIndex,scoringConfirmed:this.scoringConfirmed,importWarningsAcknowledged:this.importWarningsAcknowledged,instrumentId:this.instrumentId,customDefinition:this.customDefinition,customDraft:this.customDraft,platformImportSource:this.platformImportSource,platformImportReview:this.platformImportReview,platformImportConfirmed:this.platformImportConfirmed,platformImportSelectedGroupId:this.platformImportSelectedGroupId,platformImportSelectedRatingSetId:this.platformImportSelectedRatingSetId,studyId:this.studyId,studyTitle:this.studyTitle,taskLabel:this.taskLabel,participantCode:this.participantCode,showScoreToParticipant:this.showScoreToParticipant,showSimpleLanguage:this.showSimpleLanguage,answerMode:this.answerMode,largeText:this.largeText,audioGuidance:this.audioGuidance,recoveryEnabled:this.recoveryEnabled,participantAdjustmentPolicy:this.participantAdjustmentPolicy,voiceInputAvailable:this.voiceInputAvailable,gazeInputAvailable:this.gazeInputAvailable,collectionMode:this.collectionMode,qualtricsSurveyUrl:this.qualtricsSurveyUrl,generatedConfig:this.generatedConfig}))}catch{}}restoreConductorDraft(){try{const e=window.sessionStorage.getItem(_e);if(!e){this.draftRestored=!0;return}const t=JSON.parse(e),n=t.setupRoute==="imported"?"imported":"ready-made";this.setupRoute=n;const i=n==="imported"?Ye:Ve,r=Number(t.wizardStepIndex);this.wizardStepIndex=Number.isInteger(r)&&r>=0&&r<i.length?r:0,this.scoringConfirmed=t.scoringConfirmed===!0,this.importWarningsAcknowledged=t.importWarningsAcknowledged===!0,typeof t.instrumentId=="string"&&(this.instrumentId=t.instrumentId),t.customDefinition&&typeof t.customDefinition=="object"&&(this.customDefinition=t.customDefinition),t.customDraft&&typeof t.customDraft=="object"&&(this.customDraft=t.customDraft),typeof t.platformImportSource=="string"&&(this.platformImportSource=t.platformImportSource),t.platformImportReview&&typeof t.platformImportReview=="object"&&(this.platformImportReview=t.platformImportReview),this.platformImportConfirmed=t.platformImportConfirmed===!0,typeof t.platformImportSelectedGroupId=="string"&&(this.platformImportSelectedGroupId=t.platformImportSelectedGroupId),typeof t.platformImportSelectedRatingSetId=="string"&&(this.platformImportSelectedRatingSetId=t.platformImportSelectedRatingSetId),typeof t.studyId=="string"&&(this.studyId=t.studyId),typeof t.studyTitle=="string"&&(this.studyTitle=t.studyTitle),typeof t.taskLabel=="string"&&(this.taskLabel=t.taskLabel),typeof t.participantCode=="string"&&U(t.participantCode)&&(this.participantCode=t.participantCode),this.showScoreToParticipant=t.showScoreToParticipant===!0,this.showSimpleLanguage=t.showSimpleLanguage===!0,this.answerMode=t.answerMode==="smiley"?"smiley":"standard",this.largeText=t.largeText===!0,this.audioGuidance=t.audioGuidance===!0,this.recoveryEnabled=t.recoveryEnabled!==!1,(t.participantAdjustmentPolicy==="locked"||t.participantAdjustmentPolicy==="presentation-only"||t.participantAdjustmentPolicy==="participant-choice")&&(this.participantAdjustmentPolicy=t.participantAdjustmentPolicy),this.voiceInputAvailable=t.voiceInputAvailable!==!1,this.gazeInputAvailable=t.gazeInputAvailable===!0,this.collectionMode=t.collectionMode==="qualtrics"?"qualtrics":"local",typeof t.qualtricsSurveyUrl=="string"&&(this.qualtricsSurveyUrl=t.qualtricsSurveyUrl);const s=t.generatedConfig?Me(t.generatedConfig):null;s&&this.useConfiguration(s),n==="imported"&&(this.platformImportReview?.requiresGroupSelection||this.platformImportReview?.requiresRatingSetSelection)&&!this.platformImportContents&&(this.wizardStepIndex=1,this.platformImportReview=null,this.platformImportSelectedGroupId="",this.platformImportSelectedRatingSetId="",this.message="Select the source file again to continue choosing its LimeSurvey group or rating set.")}catch{window.sessionStorage.removeItem(_e)}finally{this.draftRestored=!0}}validateStudyDetails(){Oe({instrumentId:this.instrumentId,...this.customDefinition?.id===this.instrumentId?{questionnaireDefinition:this.customDefinition}:{},studyId:this.studyId,studyTitle:this.studyTitle,taskLabel:this.taskLabel,showScoreToParticipant:this.showScoreToParticipant,support:this.currentSupportConfig(),collection:{mode:"local"}})}render(){return l`
      ...
      <main class="app-shell conductor-shell" id="conductor-main">
        ...
        >
          ...
          <div class="form-grid" ?hidden=${this.wizardStep.key!=="source"||this.setupRoute!=="ready-made"}>
            ...
            <aside
              ...
              tabindex="-1"
              aria-describedby=${this.definitionConfirmation?"definition-confirmation-message":C}
            >
            ...
            ${this.customBuilderOpen?this.renderCustomQuestionnaireBuilder(!1):C}
          </div>
        ...
        >
          ...
          ${this.generatedConfig?l`<div
                ...
                aria-describedby=${this.configurationConfirmation?"configuration-confirmation-message":C}
                tabindex="-1"
              >
                ...
                ${U(this.participantCode)&&this.participantUrl?l`
                      ...
                      <div class="button-row compact">
                        ...
                        ${this.generatedConfig.collection.mode==="local"?l`<a class="secondary-button link-button" href=${this.participantUrl} target="_blank" rel="noopener">Open participant page</a>`:C}
    ...
    `}renderQuestionnaireReviewStep(){const e=this.wizardStep.key;if(!["upload","questions","answers","warnings","scoring"].includes(e))return C;if(e==="scoring"&&this.setupRoute==="ready-made")return l`
        <section class="panel conductor-panel" aria-labelledby="questionnaire-scoring-heading">
          ...
          <label class="platform-import-final-confirmation">
            <input
              ...
              @change=${n=>{this.scoringConfirmed=n.currentTarget.checked}}
            />
            <span>
        ...
        <section class="panel conductor-panel" aria-labelledby="import-question-review-heading">
          ...
          <ol class="wizard-review-list">
            ${this.customDraft.items.map(n=>l`<li>
              <strong>${n.name||"Unnamed item"}</strong>
              <span>${n.prompt||"No question text was found."}</span>
              <small>Source key: ${n.key} · required single answer</small>
            </li>`)}
          </ol>
        </section>
      `;if(e==="answers"){const n=[];for(let i=this.customDraft.minimum;i<=this.customDraft.maximum;i+=this.customDraft.step)n.push(i);return l`
        <section class="panel conductor-panel" aria-labelledby="import-answer-review-heading">
          ...
          <dl class="study-details">
            ...
            <div><dt>Stored values</dt><dd>${n.join(", ")}</dd></div>
          ...
          <ol class="wizard-review-list compact-review-list">
            ${this.customDraft.items.map(i=>l`<li>
              ...
              <span>${i.lowAnchor} (${this.customDraft.minimum}) → ${i.highAnchor} (${this.customDraft.maximum})</span>
              ${i.responseLabels?l`<small>Visible labels: ${Object.entries(i.responseLabels).map(([r,s])=>`${r} = ${s}`).join("; ")}</small>`:l`<small>Intermediate positions use their stored number.</small>`}
            </li>`)}
          </ol>
        </section>
      `}return e==="warnings"?l`
        <section class="panel conductor-panel" aria-labelledby="import-warning-heading">
          ...
          <label class="platform-import-final-confirmation">
            <input
              ...
              @change=${n=>{this.importWarningsAcknowledged=n.currentTarget.checked}}
            />
      ...
      <section class="panel conductor-panel" aria-labelledby="import-scoring-heading">
        ...
        <div class="form-grid">
          <label>
            ...
            <input
              ...
              @input=${n=>this.updateCustomDraft("language",n.currentTarget.value)}
            />
          </label>
          <label>
          ...
          <label>
            ...
            <input
              ...
              @input=${n=>this.updateCustomDraft("scoreName",n.currentTarget.value)}
            />
          </label>
        </div>
        <fieldset class="platform-import-reverse-items">
          ...
          ${this.customDraft.items.map((n,i)=>l`<label>
            <input
    ...
    `}renderImportFindingList(e,t,n,i){return l`<section class=${`platform-import-finding ${t}`}>
      <h3>${e} (${n.length})</h3>
      ${n.length?l`<ul>${n.map(r=>l`<li>
    ...
    `}renderCustomQuestionnaireBuilder(e=!0){return l`
      ...
      >
        ...
        ${e?this.renderPlatformQuestionnaireImport():C}
        ...
        <fieldset class="custom-items">
          ...
          ${this.customDraft.items.map((t,n)=>this.renderCustomQuestionnaireItem(t,n))}
          <button
            class="secondary-button"
            type="button"
    ...
    `}renderPlatformQuestionnaireImport(e=!0){const t=this.platformImportReview;return l`
      ...
      >
        ...
        <div class="form-grid">
          <label>
            ...
            <select
              ...
              @change=${n=>{this.platformImportSource=n.currentTarget.value,this.platformImportReview=null,this.platformImportConfirmed=!1,this.importWarningsAcknowledged=!1,this.platformImportSelectedGroupId="",this.platformImportSelectedRatingSetId="",this.platformImportContents="",this.platformImportFileName=""}}
            >
        ...
        ${t&&(e||!t.canConvert||t.requiresGroupSelection||t.requiresRatingSetSelection)?this.renderPlatformQuestionnaireReview(t):C}
      </section>
    `}renderPlatformQuestionnaireReview(e){const t=(n,i,r,s)=>l`
    ...
    `}updateCustomDraft(e,t){this.customDraft={...this.customDraft,[e]:t},this.platformImportReview&&(this.platformImportConfirmed=!1),this.platformImportReview&&(this.importWarningsAcknowledged=!1)}updateCustomItem(e,t,n){this.customDraft={...this.customDraft,items:this.customDraft.items.map((i,r)=>r===e?{...i,[t]:n}:i)},this.platformImportReview&&(this.platformImportConfirmed=!1),this.platformImportReview&&(this.importWarningsAcknowledged=!1)}removeCustomItem(e){this.customDraft.items.length!==1&&(this.customDraft={...this.customDraft,items:this.customDraft.items.filter((t,n)=>n!==e)})}activateCustomDefinition(e,t){this.customDefinition=e,this.instrumentId=e.id,e.supports.simplerExplanations||(this.showSimpleLanguage=!1),this.answerMode="standard",this.generatedConfig=null,this.participantUrl="";const n=t==="imported"?"imported, validated and selected":"validated and selected";this.definitionConfirmation=`${e.name} ${e.version} ${n}. Complete the study details, then generate a new configuration.`,this.configurationConfirmation="",this.message="",this.revealConductorResult("#selected-questionnaire-summary")}booleanOption(e,t,n,i=""){return l`<label class="toggle-card conductor-toggle">
      <input type="checkbox" .checked=${t} @change=${r=>n(r.currentTarget.checked)} />
      <span><strong>${e}</strong>${i?l`<small>${i}</small>`:C}</span>
