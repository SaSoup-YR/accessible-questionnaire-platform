...
describe('RF-06 speech listening lifecycle', () => {
  it('primes one live status and exposes an explicit Stop control while Listening', async () => {
  ...
  it('ends a browser that never fires onend with the AQP watchdog and preserves the answer', async () => {
    vi.useFakeTimers();
    const instances = installHangingRecognition();
    const component = await renderComponent();
    await startRatings(component);
  ...
  it('turns a native no-speech end into a specific safe next action and cancels the watchdog', async () => {
    vi.useFakeTimers();
    const instances = installHangingRecognition();
    const component = await renderComponent();
    await startRatings(component);
