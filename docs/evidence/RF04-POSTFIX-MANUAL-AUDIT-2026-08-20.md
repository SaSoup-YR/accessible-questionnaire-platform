...
## Recorded route environment

The manual note records the following environment details. Missing version fields are left missing rather than inferred.

| Route | Recorded environment |
| --- | --- |
| R1 | Windows 11 Home, Version 25H2, OS build 26200.9168; Firefox 154.0 (64-bit); NVDA used, exact NVDA version not separately transcribed in the retained note. |
| R2 | NVDA + Chrome on the Windows audit machine; Chrome 151.0.7922.138 (Official Build) (64-bit). |
| R3 | macOS 15.4.1, build 24E263; Safari 18.4 (20621.1.15.11.10); VoiceOver built into macOS 15.4.1. |
| R4 | Windows Voice Access + Chrome; Chrome 151.0.7922.138 (Official Build) (64-bit); Voice Access version not separately exposed in the retained note. |

The later VoiceOver `VO+Fn+F4` / keyboard-focus query attempts are **not** used as decisive R3-A14 evidence because, on the tested Mac, VoiceOver exposed the function-key control itself as `F4 button`. The R3-A14 adjudication instead uses the directly observed post-reload VoiceOver interaction: with VoiceOver enabled, the route did not land directly on Resume and manual VoiceOver navigation was required.
...
| R2 NVDA + Chrome | A15 regression | **Observed consistent with prior pass** | Resume continued to Rating 4 of 10 / Item 4 with Item 4 heading focus and no Item 4 answer. The retained manual note did not separately transcribe a second 5/1/4 retention check for R2, so no new closure claim depends on this route. |
| R3 VoiceOver + Safari | A14 | **F** | With VoiceOver enabled, reload did not place the VoiceOver cursor/accessibility focus directly on `Resume saved questionnaire`; manual VoiceOver navigation was required. A visible focus indication observed without VoiceOver, browser DOM focus, and automated WebKit focus checks were not substituted for the named VoiceOver + Safari route requirement. |
