...
| F5 | Real-browser axe scans run in CI and publish a report. | Playwright specification, `verify.yml`, JSON/HTML publisher and uploaded Playwright traces/results. | Frozen S01–S27 inventory × five profiles = 135 required rows; missing rows, violations, incomplete checks, overflow and target-size failures are reported. | CI-pass on candidate `4fe422c` in run `31399712181`: 27 states × 5 profiles = 135/135 rows; 0 automatically detected axe violations; 13 incomplete `color-contrast` results involving 20 node occurrences retained for manual inspection; 0 horizontal-overflow failures; 0 critical target-size failures; and 0 missing state/profile combinations. Retained rendered artifact `9067181758`, SHA-256 `52b4631a98d03857d10356632bf09a3c7ac3627a1f7dca822b9052f2a02a8adb`. This is bounded browser automation, not a complete WCAG or assistive-technology usability result. | Manually determine and record every overlap-dependent contrast occurrence; repeat the automated run on final main/release SHA. |
