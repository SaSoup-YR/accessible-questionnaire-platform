...
| --- | --- | --- |
| Review showed only item number and numeric value. | Every review card now shows the item statement, stored value, input route and the questionnaire's declared response meaning. A declared label is shown for that value; where an instrument such as SUS declares only endpoint anchors, the card shows those endpoints instead of inventing an intermediate label. Each card has a unique visible **Change item N answer** control whose accessible name starts with the same wording and which is exposed to the gaze route. Editing is transactional: selecting a proposed replacement does not alter the canonical response, recovery record, input route or score until Save; Cancel truthfully retains the original and returns focus to that review item. | The component regression uses Brooke's `5, 1, 4, 2, 3, 5, 1, 4, 2, 3` vector and directly checks Mark's Item 3 = 4 example, the declared endpoint context, value, route, score and recovery storage before edit, during a proposed change and after Cancel. The rendered-browser route checks the same Item 3 context plus edit/focus/Save/Cancel. Manual A16/A17/A21, fresh PR CI and deployed-build verification remain required. |
