...
| --- | --- | --- |
| Review showed only item number and numeric value. | Every review card now shows the item statement, stored value with its visible answer label and input route. Each card has a unique visible **Change item N answer** control whose accessible name starts with the same wording and which is exposed to the gaze route. Editing is transactional: selecting a proposed replacement does not alter the canonical response, recovery record, input route or score until Save; Cancel truthfully retains the original and returns focus to that review item. | Component regression checks value, route, score and recovery storage before edit, during a proposed change and after Cancel; rendered-browser review/edit/focus/Save/Cancel checks run in CI. Manual A16/A17/A21 and deployed-build verification remain required. |
