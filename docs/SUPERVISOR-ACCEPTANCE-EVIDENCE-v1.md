...
| --- | --- | --- | --- | --- | --- |
| F1 | Review shows complete item text, selected answer label and a per-item Change control. Change opens the selected item and returns to the same review record. | `source/src/accessible-nasa-tlx.ts`; review/edit state and focus-return code | `source/tests/component.test.ts`; `source/tests/study-component.test.ts`; `source/tests/e2e/browser-accessibility.spec.ts` SUS review/edit route | Local unit/component suite passed. The Playwright specification is discovered but has not yet run in release CI. | Passing Playwright review state and edit-return trace for the release SHA; deployed review smoke check. |
...
| F4 | A stable definition hash is carried in configuration and result, recomputed at load and submission, and a mismatch blocks success. | `source/src/study.ts`; `source/src/result-sink.ts`; configuration/result/CSV/Qualtrics fields | `source/tests/study.test.ts`; `source/tests/study-component.test.ts`; `source/tests/result-sink.test.ts`; `source/tests/technical-evaluation.test.ts` | Local tampered-link, submission-block and reconstruction checks passed. | CI report shows hash checks and reconstruction for the release SHA; deployed tamper test cannot submit or show false success. The hash is an unsigned consistency fingerprint, not proof of researcher origin. |
| F5 | axe scans run in a real browser in CI over all required runner states and reports are published. | `.github/workflows/verify.yml`; `source/playwright.config.ts`; `source/tests/e2e/browser-accessibility.spec.ts`; report publisher | Four Playwright scenarios cover 12 named states under five profiles: 1280, 768, 320 CSS pixels, CDP 200% zoom and a 640×450 companion viewport, for 60 required scans. Tests also check overflow, target size and rendered focus. | Test discovery and TypeScript build pass locally. No browser executable is installed in the local environment, so zero real-browser results are claimed. | GitHub Actions installs Chromium/Firefox/WebKit, runs the suite and publishes `rendered-accessibility-evidence` containing axe JSON/HTML, Playwright HTML/traces and support matrix, all tied to the release SHA. |

## Endpoint-label PS

| Requirement | Code/test evidence | Current status | Final evidence |
| --- | --- | --- | --- |
| Retain the fieldset legend as the announced endpoint source; keep the visual anchor `aria-hidden`; suppress an option label only when it exactly duplicates its endpoint; preserve distinct middle labels on a five-point German scale. | `source/src/accessible-nasa-tlx.ts` (`visibleResponseLabel`); `source/tests/platform-component.test.ts`; `source/tests/e2e/browser-accessibility.spec.ts` German five-point route | Unit/component test passed; Playwright test is discovered. | Passing rendered-browser test plus deployed visual and accessibility-tree inspection. |
...
| L2a | Programmatic fidelity round trip for all four built-ins and every QSF/LSS/LSG/LSQ fixture against an independent source of truth. | `docs/evidence/fidelity-source-of-truth.json`; `source/tests/technical-evaluation.test.ts`; `docs/evidence/technical-evaluation-report.json/.html` | 9 cases; 39 items; 288 field comparisons; 0 mismatches. | Locally evidenced; release-CI reproduction pending. |
...
| L2c | Real-browser WCAG scans in CI at 1280, 768 and 320 widths and 200% zoom, with every named state and a published artefact. | Browser specification, Playwright configuration and `verify.yml` | 60 scans pre-specified; no local rendered result claimed. | Not evidenced until GitHub Actions publishes the artefact. |
| L2d | Cross-browser support matrix, explicitly recording Firefox/Web Speech limitations. | `source/tests/e2e-support/cross-browser-support.spec.ts`; `docs/CROSS-BROWSER-SUPPORT-MATRIX.md`; browser-support report publisher | Chromium, Firefox and Playwright WebKit projects discovered. The protocol distinguishes Linux WebKit from Safari/VoiceOver. | Runtime result pending CI. |
| L2e | Reconstruct original instrument and response set from the result export alone. | Technical test and report; embedded complete definition in each result record | 9/9 exports and 39/39 responses reconstructed; 0 mismatches. | Locally evidenced; release-CI reproduction pending. |
...
- 19/19 test files and 194/194 tests passed;
- 9 fidelity cases, 39 items and 288 comparisons with 0 mismatches;
- 12 adversarial inputs with 0 silently altered;
- 9/9 exports and 39/39 responses reconstructed with 0 mismatches;
